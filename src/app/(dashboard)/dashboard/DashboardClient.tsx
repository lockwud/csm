...
type ApiResult = { ok: true; data: DashboardMetrics } | { ok: false; error: string };

export function DashboardClient({ initialData }: { initialData: DashboardMetrics }) {
  ...
  return (
    <div className="grid gap-6">
      ...
      {metrics.databaseUnavailable ? (
        <div className="rounded-md border border-warning/30 bg-warning/10 px-4 py-3 text-sm font-semibold text-text">
          Connection is unavailable. Showing empty dashboard data until the database can be reached.
        </div>
      ) : null}
