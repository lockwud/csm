...
export default async function FinancePage() {
  ...
  return <div className="grid gap-5"><h1 className="text-2xl font-bold">Finance</h1><Card><CardHeader><CardTitle>Finance Entries</CardTitle></CardHeader><CardContent className="p-0"><Table><THead><TR><TH>Reference</TH><TH>Party</TH><TH>Type</TH><TH>Amount</TH><TH>Status</TH></TR></THead><TBody>{(entries as FinanceEntryRow[]).map((entry: FinanceEntryRow) => <TR key={entry.id}><TD className="font-bold">{entry.reference}</TD><TD>{entry.party}</TD><TD>{entry.type.replaceAll("_", " ")}</TD><TD>{formatCurrency(String(entry.amount))}</TD><TD>{entry.status}</TD></TR>)}</TBody></Table></CardContent></Card></div>;
}
